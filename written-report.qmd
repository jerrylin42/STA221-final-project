...
Therefore, below we have decided on the follow variables for our final model:
...
\end{align*}
```
As we were testing this model and checking assumptions, we noticed some concerning evidence of data entry errors. We dove deeper into our points that had high standardized residuals and began to look for a pattern in the data. After not finding anything to indicate that a certain genre, language, or runtime length may be causing some sort of abnormality, we looked to log_revenue and log_budget. Interestingly, we found that the ratio between log_revenue and log_budget tended to be much more extreme for our influential points as compared to the average of the entire data set. As such, we looked at a few specific data points to see if there were potential issues; we found data entry errors were present. For instance, the movie *Kiss of the Spider Woman* has a reported budget of \$11, but according to the American Film Institute, the actual budget was north of \$1.5 million. When briefly looking at a few other data points, this was the case for multiple other movies with extreme log_revenue to log_budget ratios as well.

As such, we decided to remove these points and create a new data set for our final model. We checked linear regression assumptions for the data set without these points, which can be found in the appendix (Appendix Figures 8 and 9).
...
The constant variance condition appears to be almost satisfied, and the plot less skewed than the constant variance plot for the model with obvious data entry errors present (Appendix Figure 8).
...
```{r}
tidy(final_model_nooutliers) |>
  kable(digits = 3)

glance(final_model_nooutliers) |> select(r.squared, adj.r.squared, AIC, BIC)
```

When fitting our final model based on the data without the likely data entry error points present, we saw that our $R^2$ value increased by nearly 4%. Additionally, it appears that our constant variance condition is closer to being satisfied than in the previous model. We believe this is more than enough evidence to suggest that our model does a better job of predicting movie revenue when movies with extreme log_revenue to log_budget ratios are not present in the data, because they are likely the result of data entry errors. It is important to note that ultimately, removing all these movies is not perfect, as it is entirely possible that a movie may have a very low budget and very high revenue, or vice versa.

\^\^ THIS NEEDS TO BE ADJUSTTED TO REMOVE CONSTANT VARIANCE, ETC.

\>\> BELOW, NEED TO MAKE SURE THIS FITS FINAL MODEL DATA
...
-   When vote_average is 0, budget is 1, runtime is 0, year is 0, multi_genre is FALSE, we would expect the movie to make exp(24.8) dollars. However, this interpretation is not relevant because there are variables like year and log_budget that would never be zero, particularly since we filtered out movies that were below a nonzero threshold for both these variables.

-   For a one unit increase in the average score given to the movie, we would expect the revenue to multiply by 1.8, holding all other variables constant.
...
-   We would expect a movie with more than genre to make .3 times the revenue of a single genre movie, if all other variables were the same.
...
From our analysis, the most influential factors were the average audience rating (vote), budget, runtime, release year, and whether the movie was categorized as having multiple genres. These metrics provided meaningful insights into what contributes to a movie’s box office performance. However, it is also important to recognize what is missing from our data. For example, not all movies are competing at the box office. Streaming platforms like Netflix and Hulu offer viewers the convenience of watching from home, which has reshaped how success is defined in the film industry. Moreover, our dataset only focused on widely popular films, which means smaller, independent movies that did not achieve mainstream attention were excluded. This could have introduced potential bias in our results.

Ultimately, understanding what influences box office revenue is a powerful tool for producers and studios. By focusing on factors like how well people like the movie, how much money is spent making it, and how many genres it covers, they can make more informed decisions during production. At the same time, they should stay mindful of emerging trends in the entertainment industry, like streaming services, that continue to reshape what makes a movie successful. Overall, with success no longer defined solely by ticket sales, producers can explore new ways to measure and achieve it.
