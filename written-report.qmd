...
# should we do anything about these points? common theme amongst points that have high .std.resid is that they have log_revenue to log_budget ratios that are very different from the mean  

# see below how 49 of the 61 total points with ratios that are 3 st. devs. away from the mean also happen to have .std.resid that are 3 st. devs. away from the mean
...
```{r}
# final model no outliers test

nooutliers_model <- lm(data = nooutliers, log_revenue ~ vote_average + log_budget + runtime + year + multi_genre + multi_genre*runtime)

tidy(nooutliers_model) |>
  kable(digits = 3)

glance(nooutliers_model) |> select(r.squared, adj.r.squared, AIC, BIC)
```
