...
plot1 = movies |>
  ...
  geom_histogram(bins = 40) + 
  labs(
    title = "Histogram of Movie Budgets",
...
plot2 = movies |>
  ...
  geom_boxplot() + 
  labs(
    title = "Boxplot of Movie Budgets",
    x = "Budget (in millions of dollars)"
  ) +
  scale_x_continuous(labels = scales::dollar_format(scale = 1, suffix = "M"))
...
movies |>
  summarise(
    min = min(budget / 1e6, na.rm = TRUE), 
    q1 = quantile(budget / 1e6, 0.25, na.rm = TRUE), 
    median = median(budget / 1e6, na.rm = TRUE), 
    q3 = quantile(budget / 1e6, 0.75, na.rm = TRUE), 
    max = max(budget / 1e6, na.rm = TRUE), 
    mean = mean(budget / 1e6, na.rm = TRUE), 
    sd = sd(budget / 1e6, na.rm = TRUE),
    IQR = q3 - q1
  ) |>
  kable(digits = 3)
