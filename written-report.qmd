...
\end{align*}
```
As we were testing this model and checking assumptions, we noticed some concerning evidence of data entry errors. We dove deeper into points that had high standardized residuals and began to look for a pattern in the data. After not finding anything to indicate that a certain genre, language, or runtime length may be causing some sort of abnormality, we looked to log_revenue and log_budget. Interestingly, we found that the ratio between log_revenue and log_budget tended to be much more extreme for points with high standardized residuals, as compared to the average of the entire data set. As such, we looked at a few specific data points to see if there were potential issues; we found data entry errors were present. For instance, the movie *Kiss of the Spider Woman* has a reported budget of \$11, but according to the American Film Institute, the actual budget was north of \$1.5 million. When briefly looking at a few other data points, this was the case for multiple other movies with extreme log_revenue to log_budget ratios as well.

As such, we decided to remove these points and create a new data set for our final model. In total, 61 movies were removed. We checked linear regression assumptions for the data set without these points, which can be found in the appendix (Appendix Figures 8 and 9). The key difference is that constant variance was not satisfied before, but it appears to be mostly satisfied now.
...
The constant variance condition appears to be mostly satisfied, and the plot is much less skewed than the constant variance plot for the model with obvious data entry errors present (Appendix Figure 8). Our model meets the linearity assumption because we cannot predict with strong accuracy if a residual will be positive or negative.
...
As a whole, we conclude that our model meets the linearity, constant variance, and normality requirements, but fails to meet the assumption of independence in a few limited cases.
